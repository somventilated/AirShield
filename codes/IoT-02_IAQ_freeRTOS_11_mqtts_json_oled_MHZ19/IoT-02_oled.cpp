...
void vScreenIAQ(int nEstat, int nIAQ, int nCO2) {
  ...
  display.setTextAlignment(TEXT_ALIGN_LEFT);
  display.setFont(ArialMT_Plain_10);
  display.drawString(20, 0, "CO2: " + String(nCO2) + ", IAQ: " + String(nIAQ));
  display.setFont(ArialMT_Plain_16);
  ...
  switch (nEstat) {
    ...
    case 4:
      if (nCO2 < MAX_CO2) {
        ...
        display.drawString(15, 28, "CO2 !!!!");
        display.setFont(ArialMT_Plain_10);
        display.drawString(17, 52, "CAL VENTILAR (CO2)");
      }
      break;
    case 5:
      if (nCO2 < MAX_CO2) {
...
void vPantallaHiHaWiFi() {
  ...
  display.setTextAlignment(TEXT_ALIGN_LEFT);
  display.setFont(ArialMT_Plain_24);
  display.drawString(0, 20, "Hi ha WiFi");
  display.display();
}
